...
while True:
    ...
    try:
        ...
        if before_rsi < 25 and before_rsi < now_rsi and c.get_balance() == 0:
            ...
            if c.get_balance_krw() > 0:
                ...
                # 매수 한 다음 5분봉 기준 시간
                after_time = c.get_minute_bar().iloc[[199]].index[0] + datetime.timedelta(minutes=15)
        ...
        if c.get_balance() == 0:
            ...
            if after_time < datetime.datetime.now():
                ...
                if now_earning_rate < float(mp):
                    ...
                    if before_rsi < 25 and before_rsi < now_rsi:
            ...
            if c.get_balance_krw() < 5000:
                ...
                if (c.get_current_price() - avg_buy_price) / avg_buy_price * 100 < float(loss) and c.get_balance_krw() < 5000:
                    ...
                    # 텔레그램 봇으로 알림
                    bot.sendMessage(chat_id=user_id, text="전량 손절 완료")
